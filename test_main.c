...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/16 21:28:01 by tkasbari          #+#    #+#             */
/*   Updated: 2023/09/16 22:32:38 by tkasbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	fd1 = open("./input.txt", O_RDONLY );	// Ohhh_RDONLY...
	if (fd1 == -1)
	{
	...
	while (i < 4)
	{
		...
		if (!str)
		{
			printf("Error reading next line. Closing fd...\n");
			if (close(fd1) == -1)
				printf("Error closing...\n");
			return (-1);
		}
		printf("str: %s\n", str);
		fd_content = str; // append and malloc....
		printf("fd_content after %i get_next_line-calls: %s\n", i, fd_content);
		i++;
	}
